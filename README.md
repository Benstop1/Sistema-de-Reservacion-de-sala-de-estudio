...
El proyecto se ejecuta realizando una verificacion interna de la base de datos (br.txt) Si el archivo no existe, el sistema lo inicializa automáticamente creando 168 bloques horarios vacíos rellenando las lineas con la palaba "NONE". En un segundo paso se registra una reserva en el que el usuario selecciona la opción para crear una reserva para la sala de estudio. Despues el sistema despliega una lista de las 7 salas disponibles, tras elegir la sala se selecciona la fecha (Hoy o Mañana) y una de las 12 franjas horarias (de 08:00 AM a 08:00 PM). Ya para el momento de la reserva se solicita el nombre y el teléfono del usuario, al confirmar, los datos se escriben exactamente 5 líneas a cada celda horaria en el archivo. Posteriormente, en un paso 3 se consulta, modifica o elimina la reservacion. Para la búsqueda binaria, el usuario ingresa el ID de la sala y la fecha, el sistema calcula la disponibilidad de la sala y muestra únicamente las horas ocupadas en ese día específico. Para las opciones de modificar o eliminar (CRUD): El usuario selecciona la coordenada de la reserva que quiere cambiar ya dentro del sistema se cambian los datos con una nueva información o restablece los campos a "NONE" si se trata de una cancelación. Finalmente se peude hacer la auditoría y cierre que incluye lso reportes y exportaciónes. Un ejemplo de reporte es el reporte ordenado  o tambien llamado "Bubble Sort" que muestra en la pantalla de la consola una agenda cronológica completa de las salas, ordenando todas las reservas activas del sistema desde la primera hora de la mañana hasta la última de la noche. Y por ultimo se sale de todo y se sale del programa.
